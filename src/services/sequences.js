...
export function enrollLead(lead) {
  const sequence = lead.segment === 'commercial' ? 'commercial' : 'residential';
...
export async function processDueSteps({ limit = 50, now = new Date() } = {}) {
  ...
  for (const row of due) {
    ...
    const res = await sendTemplate({ template: templateFor(row.sequence, row.step), lead, quote });
